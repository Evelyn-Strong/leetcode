 ...
 * 1 <= radius <= 50
 */
public class _1620_bestCoordinate {
    ...
    public static int[] bestCoordinate(int[][] towers, int radius) {
        ...
        int maxSig = 0;
        int[] pos = new int[]{minX, minY};
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                int signals = calSig(x, y, towers, radius);
                if (signals > maxSig) {
                    pos = new int[]{x, y};
                    maxSig = signals;
                }
    ...
    private static int calSig(int x, int y, int[][] towers, int radius) {
        int signals = 0;
        for (int[] tower : towers) {
            double d = Math.sqrt(Math.pow(x - tower[0], 2) + Math.pow(y - tower[1], 2));
            if (d <= radius) signals += (int) tower[2] / (1 + d);
        }
        return signals;
    }
    ...
    public static void main(String[] args) {
        ...
        bestCoordinate(new int[][]{{0,1,2},{2,1,2},{1,0,2},{1,2,2}},1);
    }
}
